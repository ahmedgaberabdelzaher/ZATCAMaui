...
namespace EGAZT.ViewModel.NewDesignViewModel.CustomServicesViewModels
{
    ...
    {
        /* ObservableCollection<BalaghLocations> ports { get; set; }

         public ObservableCollection<BalaghLocations> Ports
         {
             get { return ports; }
         ...
         BalaghLocations selectedPort { get; set; }

         public BalaghLocations SelectedPort
         {
    ...
    /*    public async Task GetPorts()
        {
            ...
            {
                IsLoading = true;
                // var data = await _balaghServices.GetCustomPorts();
                 var data = await _inquiryService.GetCustomPorts(true);
                ...
                if (IsPolicyChecked)
                {
                    ...
                    if (!MailHasError && SelectedPort != null && (SelectedBalaghType != null&&IsBalaghTypeOOther==false||(IsBalaghTypeOOther && !String.IsNullOrEmpty(BalaghTypeOtherTxt))) && !String.IsNullOrEmpty(Balaghtx))
                    {
                        ...
                        BalaghTicket balaghTicket = new BalaghTicket()
                        {
                            ...
                            ticketcontent = Balaghtx, otherbalaghtype=BalaghTypeOtherTxt
                        };
