...
namespace EGAZT.Manager
{
    public class RMContactDetailsWebServiceManager
    {
        ...
        public async static Task<LargeTxPayerDModel> GetGAZTLargeTaxPayer()
        {
            if (CrossConnectivity.Current.IsConnected)
            {
                LargeTxPayerDModel largeTaxPayerModel = new LargeTxPayerDModel();
                string NewToken = string.Empty;
                try
                {
                    HttpClient client = new HttpClient(App.httpClientHandler);
                    Char lang = WebServiceManager.GetLangZParameter();
                    String url = Constants.GetRMContactDetails;
                    client.DefaultRequestHeaders.Add("Token", "123");
                    client.DefaultRequestHeaders.Add("ichannel", App.IncomingChannel);
                    var uri = new Uri(url + "?&$format=json");
                    HttpResponseMessage GAZTRMInfo = await client.GetAsync(uri);
                    if (GAZTRMInfo != null)
                    {
                        ...
                        String RmContactInfo = GAZTRMInfo.Content.ReadAsStringAsync().Result;
                        largeTaxPayerModel = JsonConvert.DeserializeObject<LargeTxPayerDModel>(RmContactInfo);
