...
namespace EGAZT.Services.Classes
{
    public class UserServices : IUserServices
    {
        ...
        {
            var response = await HttpManager.PostAsync(PageSettings.FasahBaseUrl+ "api/login", body, false,"",false,true).ConfigureAwait(false);
            return response;
        }
        public async Task<HttpResponseMessage> VerifyFasahLoginOtp(VerifyLoginBody body,string fasahLoginHeader)
        {
            var response = await HttpManager.PostAsync(PageSettings.FasahBaseUrl+ "api/user/login/verify", body, false, fasahLoginHeader, false, true).ConfigureAwait(false);
            return response;
        }
        public async Task<HttpResponseMessage> ResendOtp(ResendOtpBody body, string fasahLoginHeader)
        {
            var response = await HttpManager.PostAsync(PageSettings.FasahBaseUrl + "api/user/login/resendToken", body, false, fasahLoginHeader, false, true).ConfigureAwait(false);
            return response;
        }
    }
}
