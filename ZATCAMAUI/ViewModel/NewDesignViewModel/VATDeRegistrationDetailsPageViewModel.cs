...
namespace ZATCAMAUI.ViewModel.NewDesignViewModel
{
    ...
    public class VATDeRegistrationDetailsPageViewModel : BaseViewModel
    {
        ...
        public ICommand InitPageCommand { get; set; }
        #endregion


        private bool contactNameIsEnabled;
        public bool ContactNameIsEnabled
        {
            get
            {
                return contactNameIsEnabled;
            }
            set
            {
                if (contactNameIsEnabled == value) return;
                contactNameIsEnabled = value;
                OnPropertyChanged("ContactNameIsEnabled");
            }
        }
        ...
        public VATDeRegistrationDetailsPageViewModel(INavigationService navigationService, IDialogService dialogService) : base(navigationService, dialogService)
        {
            ...
            OnVatRegistrationReasonTapped = new Command(async () => await this.OnVatRegistrationReasonClicked());
            OnVatRegistrationDateTapped = new Command(async () => await this.OnVatRegistrationReasonDateClicked());
                ...
                return new Command(async () =>
                {
                    try
                    {
                        ...
                        if (VATDeRegistrationDetailsData != null)
                        {
                            ...
                            {
                                if (string.IsNullOrWhiteSpace(VATDeRegistrationDetailsData.d.headerSet.Fbnumx))
                                {
                                    VoidIsVisible = false;
                                }
                                else
        ...
        private void MessagingCenterCallBacks()
        {
            MessagingCenter.Subscribe<VATDeRegistrationInstructionsPageViewModel, bool>(this, "SelectedCheckboxItem", (sender, arg) =>
            {
                if (arg)
                {
                    IsInstructionChecked = arg;

                }
            });
            MessagingCenter.Subscribe<PickerPageView, GenericPickerModel>(this, "PickerSelectedItem", (sender, arg) =>
            {
                IsContactPersonEnabled = false;
                if (App.IsArabic)
                {
                    if (arg.PickerTitle.Contains(AppResources.VatDeregIDType))
                    {
                        IDType = arg.SelectedValue;

                        if (IDType.Contains(AppResources.ZZGCCID))
                        {
                            IsContactPersonEnabled = true;
            ...
            MessagingCenter.Subscribe<CalendarPickerPageView, GenericDatePickerModel>(this, "DatePickerSelectedItem", async (sender, arg) =>
            {
                ...
                if (App.IsArabic)
                {
                    ...
                    {
                        FromDate = DateTimeHelper.DateTimeFormater(arg.SelectedValue);
                    }
                    else if (arg.PickerId == "EndDateTypePicker")
                    {
                        ToDate = DateTimeHelper.DateTimeFormater(arg.SelectedValue);
                    }
                    else
                    {
                ...
                {
                    bool isValidated = await suspendedDateValidation();
                }
        ...
        public async Task ValidateIDNumberContact()
        {
            ...
            {
                ContactNameIsEnabled = false;

                if (!string.IsNullOrEmpty(TxtIDNumber))
                {
                    try
                    {

                        string Result = await TaxEvasionWebServiceManager.GAZTVATSignUpValidateIDTypesStringResp("ZS0001", TxtIDNumber, dob);
                        ...
                        {
                            ContactPersonName = vATSignUpData.d.taxpayerFullName; ;
                            ContactNameIsEnabled = false;
                            FrameIDError = false;
                        }
                    }
                    catch
                    {
                        try
                        {
                            string Result = await WebServiceManager.GAZTValidateIDTypes("ZS0001", TxtIDNumber, dob);
            ...
            if (IDType == AppResources.ZZIqamaID)
            {
                ContactNameIsEnabled = false;
                ...
                if (!string.IsNullOrEmpty(TxtIDNumber))
                {
                    try
                    {
                        ...
                        string Result = await TaxEvasionWebServiceManager.GAZTVATSignUpValidateIDTypesStringResp("ZS0002", TxtIDNumber, dob);
                        VATSignUp vATSignUpData = new VATSignUp();
                        vATSignUpData = JsonConvert.DeserializeObject<VATSignUp>(Result);
                        if (vATSignUpData.d == null)
                        {
                        ...
                        {
                            ContactPersonName = vATSignUpData.d.name1 + " " + vATSignUpData.d.name2;
                            ContactNameIsEnabled = false;
                            FrameIDError = false;
                        }
                    }
                    catch
                    {
                        try
                        {
                            string Result = await WebServiceManager.GAZTValidateIDTypes("ZS0002", TxtIDNumber, dob);
                            IDTypeValidateRootObject SignupIsIDTypeValid = JsonConvert.DeserializeObject<IDTypeValidateRootObject>(Result);
                            if (SignupIsIDTypeValid.error.message.value == "An exception was raised.")
                            {
                                FrameIDError = true;
        ...
        private async Task GetLastICRDate()
        {
            string reqType = string.Empty;
        ...
        public async Task OnVatRegistrationReasonDateClicked()
        {
            try
            {
        ...
        public async Task<bool> suspendedDateValidation()
        {
            try
            {
                ...
                {
                    if (DateTimeHelper.DateTimeFormater(LastIcrDate).Date > FromDate.Date)
                    {
                    ...
                    {
                        string validateSuspendedDate = VATDeregistrationWebServiceManager.GAZTGETVATDeregReturnFilingDateList(FromDate, ToDate);
                        ...
                        if (obj.d != null)
                        {
                            ...
                            {
                                DateTime date = DateTime.Parse(obj.d.dateResults[0].SuspDtfrom, new CultureInfo("en-US"));
                            ...
                            {
                                DateTime date = DateTime.Parse(obj.d.dateResults[0].SuspDtto, new CultureInfo("en-US"));
                            ...
                            {
                                DateTime date = DateTime.Parse(obj.d.dateResults[0].NextDtfrom, new CultureInfo("en-US"));
                            ...
                            if (obj.d.dateResults[0].NextDtfrom != null)
                            {
                                DateTime date = DateTime.Parse(obj.d.dateResults[0].NextDtto, new CultureInfo("en-US"));
                            ...
                            if (obj.d.dateResults[0].Duedate != null)
                            {
                                ...
                                NextFilingDueDate = $"{date.Year}/{date.Month}/{date.Day}";
                            }
        ...
        public async Task AddOutletDocumentOptions()
        {
            try
            {
        ...
        public async Task ReasonContinueBtnClicked()
        {
            try
            {
                ...
                {
                    if (string.IsNullOrWhiteSpace(ReasonTitle))
                    {
        ...
        public async Task SummaryContinueBtnClicked()
        {
            try
            {
                ...
                if (response.d != null)
                {
                    await _navigationService.NavigateTo(App.VATDeregistrationSuccessPage, response);
        ...
        public async Task EnableReasonView()
        {
        ...
        public async Task EnableSummaryView()
        {
        ...
        public async Task AddAttachmentEx()
        {
            try
            {
                ...
                {
                    string[] filetypes;
                    filetypes = DependencyService.Get<IDeviceInfoZATCA>().GetAttachmentTypeStringForAll();
                    filetypes = DependencyService.Get<IDeviceInfoZATCA>().GetAttachmentTypeStringForAll();
                                                        ...
                                                        try
                                                        {
                                                            ...
                                                            else
                                                            {
                                                                if (item.Erfdt != null)
                                                                {
                                                                    ...
                                                                    item.Erfdt = DateTime.Parse(item.Erfdt, new CultureInfo("en-US")).ToString("dd-MMMM-yyyy", new CultureInfo("en-US"));
                                                                }
                                                            }
                                                        }
                                                        catch (Exception ex)
        ...
        public Task setDATA(string operation)
        {
            try
            {
                ...
                {
                    requestTyp = "S";
                    VATDeRegistrationDetailsData.d.headerSet.StartDate = FromDate.ToString("yyyy-MM-ddTHH:mm:ss", new CultureInfo("en-US"));
                    ...
                    VATDeRegistrationDetailsData.d.headerSet.NextDtfrom = NextFilingStartDate.ToString("yyyy-MM-ddTHH:mm:ss", new CultureInfo("en-US"));

                    VATDeRegistrationDetailsData.d.headerSet.NextDtto = NextFilingEndDate.ToString("yyyy-MM-ddTHH:mm:ss", new CultureInfo("en-US"));

                    if (!string.IsNullOrEmpty(NextFilingDueDate))
                    {
                        VATDeRegistrationDetailsData.d.headerSet.Duedate = ConvertDateFormat(DateTime.Parse(NextFilingDueDate, new CultureInfo("en-US")));
                    }
                }
        ...
        public async Task<VATDeRegistrationDetails> saveAsDraftVoidAPIMethodCall()
        {
            ...
            try
            {
                ...
                {
                    DateTime date = DateTime.Parse(VATDeRegistrationDetailsData.d.headerSet.Taxdt, new CultureInfo("en-US"));
                    VATDeRegistrationDetailsData.d.headerSet.Taxdt = date.ToString("yyyy-MM-ddTHH:mm:ss", new CultureInfo("en-US"));
                }
